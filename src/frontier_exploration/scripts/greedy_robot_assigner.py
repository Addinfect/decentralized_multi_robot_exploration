...
class RobotAssigner(object):
	...
	def run(self):
		...
		while not rospy.is_shutdown():
			...
			#goal_index = random.randrange(len(constant_frontiers.points))
			list_of_costs =np.array(list(map(self.calculate_cost,constant_frontiers.points, repeat(self.robots[robot_index].getPosition()))))
			goal_index = np.argmin(list_of_costs)
